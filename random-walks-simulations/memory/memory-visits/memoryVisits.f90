include "memoryVisitsModule.f90"
program memoryVisits
   !!  --------------------------------------------------------------------
   !!  |  SIMULATION PROGRAM FOR THE SIMPLE RANDOM WALKER FOR THE VISITS  |
   !!  --------------------------------------------------------------------
   !!
   use omp_lib
   use memoryVisitsModule
   implicit none
   ...
   allocate(memory(realizations,2*maxDistance+1))
   ...
   do i=1,cases
      ...
      do ii=1,steps
         ...
         !$OMP PARALLEL PRIVATE(tempRand,iii,k,countsTemp,z,l)
         if (ii==1) then
         ...
         do iii=1,realizations
            ...
            if (abs(randomWalk(iii))<=maxDistance) then
               ...
               !$OMP ATOMIC
               memory(iii,k) = memory(iii,k) + 1;
            end if

            !! STEP DYNAMICS
            call random_number(tempRand);
            if (tempRand <= probabilityTemp) then
               memoryTemp = memory(iii,:);
               call random_number(tempRand);
               z = int(tempRand*sum(memoryTemp))+1;
               l = 1;
               do while(z>=1)
                  z = z - memoryTemp(l);
                  l = l + 1;
               end do
               randomWalk(iii) = l - maxDistance - 2;
            else
               call random_number(tempRand);
   ...
   !! SIMULATION INFO
   open(1, file='../../../data/visits-distribution/information/memory_info_vis.txt', status='replace')
      do i=1,5
   ...
   !! DISTRIBUTION
   open(1, file='../../../data/visits-distribution/distributions/memory_distribution_vis.txt', status='replace')
      do i=1,2*maxDistance+1
   ...
   !! CASES
   open(1, file='../../../data/visits-distribution/information/memory_cases_vis.txt', status='replace')
      do i=1,cases
   ...
   !! VARIANCE
   open(1, file='../../../data/visits-distribution/statistics/memory_variance_vis.txt', status='replace')
      do i=1,momentsTime
   ...
   !! SKEWNESS
   open(1, file='../../../data/visits-distribution/statistics/memory_skewness_vis.txt', status='replace')
      do i=1,momentsTime
   ...
   !! KURTOSIS
   open(1, file='../../../data/visits-distribution/statistics/memory_kurtosis_vis.txt', status='replace')
      do i=1,momentsTime
   ...
   !! RAW MOMENTS
   open(1, file='../../../data/visits-distribution/statistical-moments/memory_m1_vis.txt', status='replace')
      do i=1,momentsTime
         write(1,*) s1(1,i), s1(2,i), s1(3,i), s1(4,i);
      end do
   close(1)
   open(1, file='../../../data/visits-distribution/statistical-moments/memory_m2_vis.txt', status='replace')
      do i=1,momentsTime
         write(1,*) s2(1,i), s2(2,i), s2(3,i), s2(4,i);
      end do
   close(1)
   open(1, file='../../../data/visits-distribution/statistical-moments/memory_m3_vis.txt', status='replace')
      do i=1,momentsTime
         write(1,*) s3(1,i), s3(2,i), s3(3,i), s3(4,i);
      end do
   close(1)
   open(1, file='../../../data/visits-distribution/statistical-moments/memory_m4_vis.txt', status='replace')
      do i=1,momentsTime
